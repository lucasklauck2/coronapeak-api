...
public interface PacienteRepository extends JpaRepository<Paciente, Long>{
	...
	@Query("select new br.com.coronapeak.dto.DadosRelatorioDoisDTO(P.nome , C.nome) from Paciente as P join Cidade C on C.id = P.cidade join Questionario Q on Q.id = P.questionario where Q.flagPositivado = true and (select count(*) from PacienteSintoma PS where PS.codigoPaciente = P.id) > 0 and C.nome in ('Maravilha', 'Descanso', 'Pinhalzinho', 'Chapecó', 'Itapiranga') and P.peso > 60  order by C.nome asc, P.nome desc")
	public abstract List<DadosRelatorioDoisDTO> adquirirRelatorioDois();
	
	@Query("select new br.com.coronapeak.dto.DadosRelatorioTresDTO(C.id, C.nome, count(*) as qt_susp) from Paciente P join Questionario Q on Q.id = P.questionario join Cidade C on C.id = P.cidade where Q.flagSuspeito = true and C.id = P.cidade group by C.id having count(*) < 10 order by qt_susp desc")
	public abstract List<DadosRelatorioTresDTO> adquirirRelatorioTres();
	
	@Query("select new br.com.coronapeak.dto.DadosRelatorioQuatroDTO(extract(year from age(CURRENT_DATE, P.dataNascimento)), count(*) as qt_casos) from Paciente P inner join Questionario Q on Q.id = P.questionario  where Q.flagPositivado = true group by extract(year from age(CURRENT_DATE, P.dataNascimento)) having count(*) > 20 order by qt_casos desc")
